...
This software must be thoroughly tested to ensure it will be able to complete it's task on the first try. Therefore, the software will be tested both by simulating the environment it will be running in, and using the actual rover in environments similar to where it will be deployed. The simulations will be used to ensure the software behaves as inteded when faced with obstacles such as rocks and tire tracks, while the rover tests will ensure the software can detect real obstacles using the provided sensors on the rover.
...
Used to refer to the self-driving rover. The rover must navigate to a GPS destination without any human intervention.

\subsection{CanSat}
Name given to the prototype satellites used in the ARLISS competition. The words ``satelite'' and ``rover'' also are used to refer to the same object.

\subsection{Destination}
The location specified by the event organizers which the rover must navigate to after being launched from the rocket.

\subsection{Obstacle}
Any object which may impede the rover's progress to it's destination. This includes rocks, tire tracks, plants, and uneven terrain.

\subsection{Rover}
Referred to also as ``CanSat'' and ``Satellite''. This term is used when the object is on the ground.

\subsection{Satellite}
Refered to also as ``CanSat'' and ``Rover''. This term is used when the object is in the air.
...
\begin{landscape}
	...
	\begin{ganttchart}[vgrid={draw=none,draw=none},%
		...
		\ganttbar{xx}{10}{12} \ganttnewline
		%%%%%%%%%%%%%%%%%Winter2017
		\ganttgroup{Winter 2017}{11}{20} \\
		\ganttbar{xx}{10}{15} \\
		\ganttbar{xx}{15}{16} \ganttnewline
		\ganttbar{xx}{16}{18} \ganttnewline
		\ganttbar{xx}{18}{20} \ganttnewline
		\ganttbar{xx}{20}{21} \ganttnewline
		\ganttbar{xx}{21}{24} \ganttnewline
		%%%%%%%%%%%%%%%%%Spring2017
		\ganttgroup{Spring 2017}{21}{30} \\
		\ganttbar{xx}{24}{27} \\
		\ganttbar{xx}{27}{30} \ganttnewline
		\ganttbar{xx}{25}{30} \ganttnewline
